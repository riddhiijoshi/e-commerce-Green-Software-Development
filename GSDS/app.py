...
client = MongoClient("mongodb+srv://virajkunjir:EIcPUQGUuj40CTWi@cluster0.lb7hg.mongodb.net/e-commerce?retryWrites=true&w=majority") 
...
def add_product():
    try:
        ...
        # Add the 'hits' counter variable with a default value of 0
        data['hits'] = 0
...
@app.route('/update_hits', methods=['PUT'])
def update_hits():
    try:
        ...
        # Update the hits for the specified product
        products_collection.update_one({"_id": ObjectId(product_id)}, {"$inc": {"hits": 1}})

        return jsonify({"message": "Hits updated successfully."}), 200
...
def checkout():
    try:
        # Extract user_id, products, and delivery_status from the request
        user_id = request.json.get('user_id')
        products = request.json.get('products')
        delivery_status = request.json.get('delivery_status', False)  # Set default value to False if not provided
        ...
        transaction = {
            ...
            "delivery_status": delivery_status  # Include delivery status in the transaction
        }
...
def update_delivery_status():
    try:
        # Extract transaction_id and status from the request
        transaction_id = request.args.get('transaction_id')
        status = request.args.get('status')

        # Validate transaction_id and status
        if not transaction_id:
            raise ValueError("Transaction ID is missing.")
        if status.lower() not in ['true', 'false']:
            raise ValueError("Invalid status value. Use 'true' or 'false'.")

        # Convert status to boolean
        status = status.lower() == 'true'

        # Update delivery status in the transaction collection
        ...
        if result.modified_count == 0:
            raise ValueError("Transaction not found or delivery status not updated.")
...
@app.route('/admin/transaction', methods=['GET'])
def view_all_transactions():
    return render_template('allTransaction.html')

@app.route('/all_transactions', methods=['GET'])
def get_all_transactions():
    try:
        # Query the database to get all transactions
        allTransaction = list(transaction_collection.find())
        
        # If no transactions found, return an error
        if not allTransaction:
