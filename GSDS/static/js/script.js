document.addEventListener("DOMContentLoaded", () => {
  ...
  function displaySearchResults(products) {
    ...
    if (products.length === 0) {
      ...
      products.forEach((product) => {
        ...
        cartButton.dataset.productId = product._id;
  ...
  async function addToCart(event, message) {
    ...
    if (!userId) {
      // Display alert message if user ID is not present
      alert(`Please log in or sign up to add items to your ${message}.`);
      return;
    }

    try {
      // Update product hits
      await updateProductHits(productId);
  ...
  // Function to update product hits
  async function updateProductHits(productId) {
    try {
      const response = await fetch("/update_hits", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        throw new Error("Failed to update product hits.");
      }

      console.log("Product hits updated successfully.");
    } catch (error) {
      console.error(error.message);
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleCheckout() {
    try {
      ...
      window.location.href = `/transaction?user_id=${userId}`;
    } catch (error) {
      console.error("Error:", error);
...
function displayUser(user) {
  ...
  backButton.href = "/?user_id=" + userData._id;
